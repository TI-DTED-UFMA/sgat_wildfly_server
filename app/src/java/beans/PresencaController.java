...
public class PresencaController implements Serializable {
    ...
    public void setSelectedData(){
        ...
        String idKeyString = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("idParticipacao");
        if(idKeyString!=null){
            Integer idKey = Integer.parseInt(idKeyString);
            presenca = ejbPresenca.find(idKey);
        }
    }
    
    public String delete(){
        try {
            ejbPresenca.remove(presenca);
            System.out.println("Deleting presence record for " + presenca.getPessoa().getNome() + " " + presenca.getPessoa().getSobrenome());
            FacesContext facesContext = FacesContext.getCurrentInstance();
            AtaController ataBean = (AtaController) facesContext.getApplication().getVariableResolver().resolveVariable(facesContext, "ataController");
            ataBean.createParticipationJSON(presenca.getAtividade());
        } catch (Exception ex){
