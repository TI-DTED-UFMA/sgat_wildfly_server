...
public class ResponsavelController implements Serializable {
    ...
    public String saveResponsavel(){
        try {
            ...
            List<Funcionario> funcionarios = ejbResponsavel.findByPessoa(responsavel.getPessoa());
            Funcionario funcionario;
            if(funcionarios!=null && !funcionarios.isEmpty()){
                if(funcionarios.size() > 1)
                {
                    funcionario = funcionarios.get(funcionarios.size()-2);

                    Usuario usuario = ebjUsuario.getByFuncionario(funcionario);
                    if(usuario != null)
                    {
                        usuario.setFuncionario(responsavel);
                        try{
                            ebjUsuario.edit(usuario);
                        }catch(Exception e){
                            e.printStackTrace();
                        }
                    }
