            ...
            try {
                ...
                if(content.length() > 2){
                    ...
                    for(int i=0;i<obj.length();i++){
                        ...
                        String estab = json_obj.getString("estabelecimento");
                        String prof = json_obj.getString("ocupacao");
                        JSONObject estabObj = new JSONObject(estab);
                        JSONObject ocuObj = new JSONObject(prof);
                        cbo = ocuObj.getString("codigo");
                        cnes = estabObj.getString("cnes");
                        System.out.println(cnes);
                        if(firstCnes.equals(""))
                            firstCnes = cnes;
                        if(estabObj.getBoolean("ativo"))
                        {
                            cnes = estabObj.getString("cnes");
                            ativo = true;
                            break;
                        }
                        //System.out.println("cnes " + cnes + " cbo " + cbo + " ativo " + estabObj.getBoolean("ativo"));
                    }
                    if(!ativo)
                        cnes = firstCnes;
                    
                    //System.out.println("cnes " + cnes + " cbo " + cbo + " ativo " + ativo);
                }                
            }
            catch(Exception e){
