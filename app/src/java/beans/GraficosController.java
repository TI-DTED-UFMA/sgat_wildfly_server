...
import smgt.sessions.CidadeCoordenadasFacade;
...
public class GraficosController  implements Serializable{
    ...
    @EJB
    private CidadeCoordenadasFacade ejbCoords;
    ...
    @EJB
    private TbSolicitanteUnidadeFacade sgmtUnidade;
    
    @EJB
    private TbSolicitanteEquipeFacade sgmtEquipe;
    
    @EJB
    private TbSolicitantePerfilFacade sgmtPerfil;
    
    @EJB
    private TbSolicitantePessoaFacade sgmtPessoa;
    ...
    String jsonReportProfissionais = "";
    ...
    public String getJsonReportProfissionais() {
        return jsonReportProfissionais;
    }

    public void setJsonReportProfissionais(String jsonReportProfissionais) {
        this.jsonReportProfissionais = jsonReportProfissionais;
    }
    ...
    public String createPieChartAtividadesTipoNTS(List<TipoCounter> atividadesSomadas) throws JSONException, FileNotFoundException, UnsupportedEncodingException
    {
        ...
        for(TipoCounter t : atividadesSomadas)
        {
            ...
            j.put("tipo", t.getTipo().getDescricao());
            j.put("video", t.getTotals()[0]);
            j.put("web", t.getTotals()[1]);
            j.put("pres", t.getTotals()[2]);
        ...
        jsonReportProfissionais = jSONArray.toString();
        ...
        for(Presenca p : presencas)
        {
            if(p.getLocal() != null)
            {
                ...
                if(estados.containsKey(estadoNome))
                {
                    ...
                    CidadeCoordenadas coordenadas = ejbCoords.findByCodigo(p.getLocal().getIbge());
                    if(coordenadas != null)
                    {
    ...
    public void getMunicipiosMetasProfissionais() throws JSONException, FileNotFoundException, UnsupportedEncodingException
    {
        chartTotal = 0;
        //List<Atividade> atvs = ejbAtividade.getFilteredAtividades(dt1, dt2, 4);
        getSumAtividades();
        ...
        JSONArray jSONArray = new JSONArray();
        Map municipios = new Hashtable();
        List<Presenca> presencasMunicipio = new ArrayList();
        ...
        for(Object key: sorted.keySet())
        {
            Municipio municipio = ejbMunicipio.findByIbge((int)key);
            int qtd = (int)municipios.get(key);
            
            List<TbSolicitanteUnidade> unidades =  sgmtUnidade.findByIBGE(municipio.getIbge());
            //List<TbSolicitanteEquipe> equipes = new ArrayList();
            int totalProfissionais = 0;
            int totalPartsCadastros = 0;
            for(TbSolicitanteUnidade unidade : unidades)
            {
                List<TbSolicitanteEquipe> equipesUnidade = sgmtEquipe.findByUnidade(unidade.getCodigo());
                for(TbSolicitanteEquipe equipe : equipesUnidade)
                {
                    List<TbSolicitantePerfil> perfis = sgmtPerfil.findByEquipe(equipe.getCodigo());
                    if(!perfis.isEmpty())
                    {
                        totalProfissionais += perfis.size();
                        
                            for(Presenca p : presencas)
                            {
                                if(p.getLocal() != null)
                                    if(p.getLocal().getIbge() == (int)key)
                                    {
                                        for(TbSolicitantePerfil perfil : perfis)
                                        {
                                            TbSolicitantePessoa pessoa = sgmtPessoa.findByCodigo(perfil.getPessoa());
                                            
                                            if(pessoa != null)
                                            {
                                                if(p.getPessoa().getCpf().equals(pessoa.getCpf()))
                                                   totalPartsCadastros++;
                                            }
                                        }   
                                    }
            ...
            if(totalProfissionais != 0)
                percentage = (totalPartsCadastros*100)/totalProfissionais;
            else
                percentage = -1;
            
            
            
            System.out.println("Totais profissionais " + totalProfissionais);
            JSONObject j = new JSONObject();
            j.put("municipio", municipio.getNome());
            j.put("quantidade", (int)municipios.get(key));
            j.put("qtdPartsCadastrados", totalPartsCadastros);
            j.put("qtdCadastrados", totalProfissionais);
            j.put("qtdProfs", percentage);
    ...
    public void getSumAtividades(int atvStatus)
    {
        String dt1Sql = DateUtils.dateFormatMysql(dt1);
        String dt2Sql = DateUtils.dateFormatMysql(dt2);
        List<Atividade> atvs = ejbAtividade.getFilteredAtividades(dt1, dt2, atvStatus);
    ...
    public List<String> getDistinctEstados(Atividade atv){
        List<Presenca> list = ejbPartPessoa.findByAtividade(atv);
        Map estados = new Hashtable();
        ...
        for(Presenca pe : list)
        {
            ...
            if(mun != null)
            {
                String estNome = mun.getUf().getDescricao();
                if(estados.containsKey(estNome))
                    estados.put(estNome, (int)estados.get(estNome) + 1);
                else
                    estados.put(estNome, 1);
            }
        }
    ...
    public String getGrauSatisfacaoDashboard()
    {
        List<Avaliacoes> avaliacoes = ejbAvaliacoes.findByPergunta(5);
        return getPieChartPerguntas(avaliacoes);
    }
    
    public String getObjetosViewsDashboard() 
    {
        chartTotal = 0;
        System.out.println("objetos grafico");
        List<ObjetoAprendizagem> objetos = ejbObjeto.findAll();
    ...
    public String getTop4MunicipiosDashboard()
    {
        Map municipios = new Hashtable();
        ...
        int todayNumber = DateUtils.getSeparatedDateNumbers(today)[0];
        int dateNumbers[] = DateUtils.getSeparatedDateNumbers(today);
        String date = dateNumbers[2]+ "-" + ((dateNumbers[1]<10)?"0":"") + dateNumbers[1];
        List<Atividade> atividades = ejbAtividade.findByDay(date);
        System.out.println("m " + atividades.size());
        ...
        System.out.println("mun " + rows.toString());
        return rows.toString();
    }
    
    public String getAttendanceGaugeTodatDashboard()
    {
        getAttendanceRowsCurrentMonth();
        Date today = new Date();
        int todayNumber = DateUtils.getSeparatedDateNumbers(today)[0];
        int dateNumbers[] = DateUtils.getSeparatedDateNumbers(today);
        String date = dateNumbers[2]+"-"+((dateNumbers[1]<10)?"0":"") + dateNumbers[1]+"-"+((dateNumbers[0]<10)?"0":"") + dateNumbers[0];
        ...
        for(Atividade atv : atividades)
        {
            List<Presenca> parts = ejbPartPessoa.findByAtividade(atv);
            if(parts != null )
                todayTotal += parts.size();
        }

        rows.add("[\'p/d\', "+ todayTotal +" ]");
        if(totalAtividadesMes == 0)
            rows.add("[\'m/p\', "+ (0) +" ]");
        else
            rows.add("[\'m/p\', "+ (totalPartMes/totalAtividadesMes) +" ]");
        
        
        return rows.toString();
    }
    
    public String get4MonthsSummaryDashboard() throws ParseException
    {
        List<YearMonth> months = new ArrayList();        
        YearMonth thisMonth    = YearMonth.now();
        months.add(thisMonth.minusMonths(3));
        months.add(thisMonth.minusMonths(2));
        months.add(thisMonth.minusMonths(1));
        ...
        for(YearMonth month : months)
        {
            ...
            String lastDayMonth = getDateMonthYear(monthValue, year);

            DateFormat dtFormat = new SimpleDateFormat("MM/dd/yyyy");
            dt2 = dtFormat.parse(lastDayMonth);
            String dt1Str = ( (monthValue < 10) ? "0"+monthValue : monthValue+"") + "/"+"01"+"/"+year;
            dt1 = dtFormat.parse(dt1Str);
            System.out.println("dates: "+ year+" " + dt1.toString() + " " + dt2.toString());
            getSumAtividades();
            String row = "['"+DateUtils.monthNumberToMonthName(monthValue)+"', "+filtered.size()+", "
                                                                       +presencas.size()+", "
                                                                       +estados.size()+", "
                                                                       +municipios.size()+"]";
            atvSummary[0] += filtered.size();
